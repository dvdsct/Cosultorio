<div>
    ...
    @else
                ...
                <div class="small-box bg-primary">
                    ...
                    <div class="inner">
                        <h3> {{ $paciente->perfiles->personas->dni }} - {{ $paciente->perfiles->personas->apellido }}
                            {{ $paciente->perfiles->personas->nombre }} </h3>
                ...
                <div class="small-box bg-secondary">
                    ...
                    <div class="inner">
                        <h3>{{ $medico->titulo }} {{ $medico->perfiles->personas->apellido }}
                            {{ $medico->perfiles->personas->nombre }} </h3>
            ...
            <div class="col-md-6">
                <div style="width: 100%;">
                    ...
                    <div class="input-group">
                        <input type="text" wire:model="query" wire:keydown.enter='searchV'
                            class="form-control float-right" placeholder="Medicamento">
        ...
        @if ($modalRemedios)
            <div class="modal fade show" id="modal-default" wire:ignore.self
                                    ...
                                    <table id="myTable" class="table table-hover">
                                        ...
                                        <tbody>
                                            @foreach ($vademecum as $v)
                                                <tr>
                                                    <td class="p-0"> <span
                                                            style="font-size: 12px;">{{ $v->nombre . ' ' . $v->cantidad }}
                                                        </span></td>
                                                    <td class="p-0"> <span style="font-size: 12px;">
                                                            {{ $v->droga }} </span> </td>
                                                    <td class="p-0"> <span style="font-size: 12px;">
                                                            {{ $v->cantidad }} </span> </td>
                                                    <td class="p-0" style="display: flex; justify-content:flex-end;">
                                                        <button class="btn btn-info btn-sm"
                                                            wire:click='indicacionar({{ $v->id }})'><i
        ...
        <div class="row d-flex justify-content-end">
            <a class="btn btn-warning" href="{{ route('pdf.show', $consulta->id) }}" target="_blank"><strong> Imprimir
                </strong></a>
        </div>
    @endif
</div>
